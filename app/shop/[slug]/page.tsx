...
// 統一的 proxy 產生器（可帶 slug / ig_id 以便 403 時 edge 急救）
...
export default async function ShopPage({
  ...
  if (from === "startup" || referer.includes("/startup")) {
    backHref = "/startup"; activeFeature = "初創"
  } else if (from === "service" || referer.includes("/service")) {
    backHref = "/service"; activeFeature = "服務"
  } else if (from === "shop" || referer.includes("/shop/categories")) {
    backHref = "/shop/categories"; activeFeature = "網店"
  }
        ...
        {!data?.length ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {data.map((m) => {
              ...
              const nextFrom =
                from ??
                (activeFeature === "初創" ? "startup" :
                 activeFeature === "服務" ? "service" : "shop")
              ...
              return (
                ...
                >
                  <div className="relative">
                    <IgImage
                      src={proxied(img, slug, m.ig_media_id)}               // ✅ 用正確變數
                      thumb={m.thumbnail_url ? proxied(m.thumbnail_url, slug, m.ig_media_id) : undefined}
                      alt=""
