...
// Supabase SSR client（帶 cookies，若有登入會帶 JWT，否則 anon）
async function sb() {
...
export async function POST(req: Request) {
  try {
    ...
    for (const it of items) {
      ...
      merchantTotals[it.merchant_slug][cur] = (merchantTotals[it.merchant_slug][cur] || 0) + line
    }
    ...
    // 成功：回傳 order_code，前端清空購物車並導到感謝頁
